...
| `DENSE_BACKEND` | `qdrant` | Dense ANN: `qdrant` or `turbovec` |
...
### TurboVec rollout (cut Qdrant RAM)

1. Run `turbovec-sidecar` / Docker `turbovec-index`; set `TURBOVEC_URL` (ingest dual-writes vectors).
2. Keep `DENSE_BACKEND=qdrant` until the sidecar is healthy; `POST /reindex` to rebuild from Qdrant if needed.
3. Flip `DENSE_BACKEND=turbovec`. Retune `SIMILARITY_THRESHOLD` — TurboQuant scores are not identical to Qdrant cosine.
4. Recreate the collection with `QDRANT_VECTORS_ON_DISK=true` (operator-driven drop/recreate + re-ingest or restore) so float32 leaves RAM; TurboVec remains the live ANN index.

**Sidecar search:** `POST /search` runs top-k ANN first; `score_threshold` (and proxy `SIMILARITY_THRESHOLD`) filter hits *after* that shortlist — you may get fewer than `limit` results.

**Id mapping:** ingest upserts use 32-char MD5 hex Qdrant ids. The sidecar maps the first 16 hex digits to TurboQuant uint64 keys and stores the full hex in SQLite (`*.ids.sqlite` beside the `.tvim` file).

**Persistence:** `TURBOVEC_AUTO_SAVE=true` (default) rewrites the `.tvim` file after every `/add`, `/remove`, and `/reindex`, plus on shutdown. During large bulk ingests, set `TURBOVEC_AUTO_SAVE=false` on the sidecar and call `POST /save` once when dual-write finishes to avoid heavy disk churn.

**After Qdrant collection clear:** dropping or clearing the Qdrant collection does not empty TurboVec. Run `POST /reindex` on the sidecar (or wipe `TURBOVEC_INDEX_PATH` and its `.ids.sqlite`) before serving with `DENSE_BACKEND=turbovec`.
...
| `INGEST_TURBOVEC_REINDEX` | `idle` | TurboVec rebuild from Qdrant (`off` / `each` / `idle`; dual-write is primary) |
...
| `TURBOVEC_SIDECAR_UNIT` | `turbovec-sidecar.service` | systemd unit ensured up during ingest (not stopped; dual-write) |
...
| `NOMIC_POOL_PARALLEL_PER_INSTANCE` | `16` | Planner input for `--parallel` (GPU-tier and context capped; effective value written as `NOMIC_POOL_PARALLEL`) |
...
The planner writes the pool env **before** starting `nomic-embed@.service` units. That unit loads `nomic-embed-pool.env` last so planned `NOMIC_POOL_PARALLEL` reaches `llama-server --parallel` (also synced into the scale env). `NOMIC_POOL_PARALLEL` is also capped so `-c NOMIC_CONTEXT / --parallel` still fits `INGEST_CHUNK_SIZE_TOKENS` (undersized slots cause HTTP 400 `exceed_context_size` and slow ingest retries). The admin scale job syncs ingest keys and hot-reloads the worker even when optional benches warn.
